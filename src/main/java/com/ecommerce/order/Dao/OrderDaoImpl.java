...
public class OrderDaoImpl implements OrderDao {
	...
	( 
			int quantity, float amount, boolean paymentStatus, PaymentMethod paymentMethod, Date orderedDate, String deliveryAddress,
	String deliveryStatus, boolean orderCancellationStatus, String orderCancellationReason, Date orderCancellationDate,
	int cancellationQuantity, boolean refundStatus, Date refundDate, User user,
			Item item
			) 
	{
	...
	public Transactions addTransaction(Transactions transaction) 
	{
		...
		return transactionRepo.save
				(new Transactions(t.getId(),transaction.getOrder(), new Date(),transaction.getAmount() , transaction.getTransactionType(),transaction.getPaymentMethod()));
	}
